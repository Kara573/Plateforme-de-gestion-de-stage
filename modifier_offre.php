...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE OffresStage SET titre = ?, description = ?, date_debut = ?, date_publication = ? WHERE id = ?");
    $stmt->bind_param("ssssi", $titre, $description, $date_debut, $date_publication, $offre_id);

    // Exécutez la requête
    if ($stmt->execute()) {
        $_SESSION['success_message'] = 'Offre modifiée avec succès !';
    } else {
        $_SESSION['error_message'] = 'Erreur lors de la modification: ' . $stmt->error;
    }
...
<body>
    ...
    <main>
        <form action="" method="post">
            ...
            <button type="submit" class="btn btn-primary">Modifier Offre</button>
        </form>
    </main>
